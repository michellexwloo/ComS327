...
void io_print_stat(Pokemon *pkm) {
	//mvprintw(0,0, "Found pokemon %s!", pokemon->get_species());
	mvprintw(1,0, "Pokemon stats:");
	mvprintw(2,2, "Level: %d", pkm->get_level());
	mvprintw(3,2, "Hp: %d", pkm->get_hp());
	mvprintw(4,2, "Attack: %d", pkm->get_attack());
	mvprintw(5,2, "Defense: %d", pkm->get_defense());
	mvprintw(6,2, "Special Attack: %d", pkm->get_special_attack());
	mvprintw(7,2, "Special Defense: %d", pkm->get_special_defense());
	mvprintw(8,2, "SpeedL %d", pkm->get_speed());
	mvprintw(9,2, "Move 1: %s", pkm->get_moves(0));
	mvprintw(10,2, "Move 2: %s", pkm->get_moves(1));
...
void io_pokemon() {
	...
	int manhatten_dist = (abs(world.cur_idx[dim_y] - 200) + abs(world.cur_idx[dim_y] - 200));

	if(manhatten_dist == 2) {
		min_level = 1;
		max_level = 1;
	}else if(manhatten_dist <= 200) {
		min_level = 1;
		max_level = manhatten_dist/2;
	}else if(manhatten_dist == 201) {
		min_level = 1;
		max_level = 100;
	}else {
		min_level = (manhatten_dist - 200)/2;
		max_level = 100;
	}
	wild = new Pokemon(rand() % (max_level - min_level) + min_level);

	// clear screen to show the Pokemon's information
	clear();
	mvprintw(0,0, "Found a wild pokemon %s!", wild->get_species());
	io_print_stat(wild);
...
void io_choose_pokemon() {
	// 3 random Lvl 1 Pokemon
	Pokemon *pokemon1 = new Pokemon(1);
	Pokemon *pokemon2 = new Pokemon(1);
	Pokemon *pokemon3 = new Pokemon(1);

	// Display for user to choose
	char choice;
	do{
		mvprintw(0,0, "Here're 3 Pokemons:");
		mvprintw(1,2, "1: %s", pokemon1->get_species());
		mvprintw(2,2, "2: %s", pokemon2->get_species());
		mvprintw(3,2, "3: %s", pokemon3->get_species());
		mvprintw(5,0, "Enter 1, 2, or 3 to choose...");
		refresh();
		choice = getch();
		refresh();
	...
	for(int i=0; i<6; i++) {
		world.pc.pokemon_slot[i] = NULL;
	}

	if(choice == '1') {
		world.pc.pokemon_slot[0] = pokemon1;
		delete pokemon2;
		delete pokemon3;
	}else if(choice == '2') {
		world.pc.pokemon_slot[0] = pokemon2;
		delete pokemon1;
		delete pokemon3;
	}else if(choice == '3') {
		world.pc.pokemon_slot[0] = pokemon3;
		delete pokemon1;
		delete pokemon2;
	}
	world.pc.pokemon_count++;

	clear();
	mvprintw(0,0, "You've chosen %s", world.pc.pokemon_slot[0]->get_species());
	io_print_stat(world.pc.pokemon_slot[0]);
	refresh();
	getch();
...
void io_trainer_pokemon(npc *n) {
	for(int i=0; i<n->pokemon_count; i++) {
		clear();
		mvprintw(0,0, "Pokemon #%d of %d: ", i+1, n->pokemon_count);
		io_print_stat(n->pokemon_slot[i]);
		refresh();
		getch();
	}
}