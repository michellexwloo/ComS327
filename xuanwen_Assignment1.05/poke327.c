...
void display_trainer_list() {
	...
	if (world.cur_map->total_trainers < 23) {
		bottom = (world.cur_map->total_trainers - 1);
	}

	while (loop) {
		clear();
		if (world.cur_map->total_trainers > 23) {
			mvprintw(23, 0, "Trainers list %d-%d of %d. Scroll with up or down arrow keys.", (top+1), (bottom+1), world.cur_map->total_trainers);
		} else {
			mvprintw(23, 0, "Trainers list %d-%d of %d.", (top+1), (bottom+1), world.cur_map->total_trainers);
		}
		...
		for (line = 0, top_trainer = top; line < max_disp; line++, top_trainer++){
			int r = world.pc.pos[dim_y] - npcs[top_trainer]->pos[dim_y];
			int c = world.pc.pos[dim_x] - npcs[top_trainer]->pos[dim_x];
			char npc_type = npcs[top_trainer]->symbol;

			if (r<0 && c<=0) {
				mvprintw(line, 0, "%c, %d south and %d east", npc_type, (r*-1), (c*-1));
			}
			else if (r<0 && c>0)
			{
				mvprintw(line, 0, "%c, %d south and %d west", npc_type, (r*-1), c);
			}
			else if (r>=0 && c<=0)
			{
				mvprintw(line, 0, "%c, %d north and %d east", npc_type, r, (c*-1));
			}
			else if (r>=0 && c>0)
			{
				mvprintw(line, 0, "%c, %d north and %d west", npc_type, r, c);
			}	
		}
		...
		switch (getch()) {
			...
			case KEY_DOWN:
				if (bottom < (world.cur_map->total_trainers-1)){
					top++;
					bottom++;
				}
				break;
...
void pokemon_battle(character_t *opponent) {
	print_map();
	mvprintw(0, 0, "You are in a Pokemon battle.");
...
void enter_building() {
	...
	while ((building = getch()) != '<')
	{
	}	
}

int check_move(int x, int y) {
	if((world.cur_map->map[world.pc.pos[dim_y]+y][world.pc.pos[dim_x]+x] == ter_boulder) || 
		...
		(world.cur_map->map[world.pc.pos[dim_y]+y][world.pc.pos[dim_x]+x] == ter_gate) ||  
...
int move_player() {
	...
	switch (input) {
		...
		case '>':
			if((world.cur_map->map[world.pc.pos[dim_y]][world.pc.pos[dim_x]] == ter_mart) || 
				(world.cur_map->map[world.pc.pos[dim_y]][world.pc.pos[dim_x]] == ter_center)) {
					move(0,0);
					clrtoeol();
					mvprintw(0,0, "Entering the building...");
					enter_building();
				}
			break;
		...
		default:
			move(0,0);
			clrtoeol();
			mvprintw(0,0, "Invalid command. Please try again.");
			return 4;
	}
	return 1;
}
...
void init_terminal(void) {
	...
	init_pair(POKEMART_COLOR, COLOR_RED, COLOR_BLACK);
