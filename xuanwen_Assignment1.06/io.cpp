...
void io_fly(pair_t dest){
	int x = 0, y = 0;
	mvprintw(0,0, "Enter x-coordinate: ");
	refresh();
	echo();
	curs_set(1);
	mvscanw(0,20, (char*)"%d", &x);

	mvprintw(0,0, "Enter y-coordinate: ");
	refresh();
	echo();
	curs_set(1);
	mvscanw(0,20, (char*)"%d", &y);

	while (x < -200 || x > 200 || y < -200 || y > 200) {
		mvprintw(0,0, "Only [-200,200] is accepted. Re-enter x-coordinate: ");
		refresh();
		echo();
		curs_set(1);
		mvscanw(0,52, (char*)"%d", &x);

		mvprintw(0,0, "Only [-200,200] is accepted. Re-enter y-coordinate: ");
		refresh();
		echo();
		curs_set(1);
		mvscanw(0,52, (char*)"%d", &y);
	}
