...
static void print_map()
{
  ...
  for (y = 0; y < MAP_Y; y++) {
    for (x = 0; x < MAP_X; x++) {
		// Place pc first
		if (y == world.pc.position[dim_y] && x == world.pc.position[dim_x]) {
			putchar('@');
		} else {
...
#define terrain_cost(ch, x, y) movement_cost[ch][m->map[y][x]]

void calculate_distance(map_t *m, character_type_t character){
	static path_t path[MAP_Y][MAP_X], *p;
  	static uint32_t initialized = 0;
  	heap_t h;
  	uint32_t x, y;
	...
	int (*distance)[MAP_X];
	if (character == char_hiker) {
		distance = world.hiker_dist;
		heap_init(&h, hiker_cmp, NULL);
	} else if (character == char_rival) {
	...
	distance[world.pc.position[dim_y]][world.pc.position[dim_x]] = 0;

	for (y=1; y<MAP_Y-1; y++) {
		for (x=1; x<MAP_X-1; x++) {
			if (terrain_cost(character, x, y) != INT_MAX) {
				path[y][x].hn = heap_insert(&h, &path[y][x]);
			} else {
				path[y][x].hn = NULL;
			}
	...
	while ((p = heap_remove_min(&h))) {
		...
		if ((path[p->pos[dim_y] - 1][p->pos[dim_x]].hn) &&
        	(distance[p->pos[dim_y] - 1][p->pos[dim_x]] >
         	distance[p->pos[dim_y]][p->pos[dim_x]] +
         	terrain_cost(character, p->pos[dim_x], p->pos[dim_y]))) {
		...
		if ((path[p->pos[dim_y]][p->pos[dim_x] - 1].hn) &&
        	(distance[p->pos[dim_y]][p->pos[dim_x] - 1] >
         	distance[p->pos[dim_y]][p->pos[dim_x]] +
         	terrain_cost(character, p->pos[dim_x], p->pos[dim_y]))) {
		...
		if ((path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].hn) &&
        	(distance[p->pos[dim_y] - 1][p->pos[dim_x] - 1] >
         	distance[p->pos[dim_y]][p->pos[dim_x]] +
         	terrain_cost(character, p->pos[dim_x], p->pos[dim_y]))) {
...
void print_distance(character_type_t character) {
	int (*distance)[MAP_X];
	if (character == char_hiker) {
		distance = world.hiker_dist;
	...
	for (y=0; y<MAP_Y; y++) {
		for (x=0; x<MAP_X; x++) {
			if (distance[y][x] == INT_MAX) {
				printf("   ");
			} else {
				printf(" %02d", distance[y][x] % 100);
			}
		}
		printf("\n");
	}
...
int main(int argc, char *argv[])
{
  ...
  do {
	place_pc();
	calculate_distance(world.cur_map, char_hiker);
  	fflush(stdout);
	calculate_distance(world.cur_map, char_rival);
  	fflush(stdout);

    print_map(); 
	print_distance(char_hiker);
	print_distance(char_rival);
