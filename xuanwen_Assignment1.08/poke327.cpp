...
void get_trainer_pokemon(npc *c) {
	do {
		c->pokemon_count++;
  	}while((c->pokemon_count<6) && ((rand() % 10)<6));

  	for(int i=0; i<c->pokemon_count; i++) {
		int min_level, max_level;
		int manhatten_dist = (abs(world.cur_idx[dim_y]-200) + abs(world.cur_idx[dim_x]-200));
		...
		c->pokemon_slot[i] = new Pokemon(rand() % (max_level-min_level) + min_level);
	}
}
void new_hiker()
{
